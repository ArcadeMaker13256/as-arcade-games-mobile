...
function hatDirections(pad){
  ...
  if(!left&&!right&&!up&&!down){
    ...
    for(let i=0;i<axes.length;i++){const match=states.find(([value])=>Math.abs(axisValue(pad,i)-value)<.11);if(!match)continue;const name=match[1];left=name.toLowerCase().includes('left');right=name.toLowerCase().includes('right');up=name.toLowerCase().includes('up');down=name.toLowerCase().includes('down');break}
  }
  return{left,right,up,down};
}
function gamepadDirections(pad){
  const hat=hatDirections(pad),left=gamepadButtonPressed(pad,14)||hat.left,right=gamepadButtonPressed(pad,15)||hat.right,up=gamepadButtonPressed(pad,12)||hat.up,down=gamepadButtonPressed(pad,13)||hat.down;
  return{left,right,up,down,x:left?-1:right?1:0,y:up?-1:down?1:0,xAxis:'D-pad',yAxis:'D-pad'};
}
...
function controllerEnvironmentHelp(){
  ...
  if(controllerManager.isAppleMobile&&controllerManager.standalone)return'Tap Enable Nintendo Controller, then press the D-pad and B/A buttons repeatedly. If the Home Screen web app still reports no controller, open the same address directly in Safari and test there.';
  if(controllerManager.isAppleMobile)return'Keep Safari visible, tap Enable Nintendo Controller, then press the D-pad and B/A buttons repeatedly for several seconds.';
  return'Tap Enable Nintendo Controller, then press the D-pad or a face button.';
}
function updateControllerDebug(force=false){
  const now=performance.now();if(!force&&now-controllerManager.lastDebug<180)return;controllerManager.lastDebug=now;const pads=connectedGamepads(),live=qs('#controllerLive'),debug=qs('#controllerDebug');if(!live||!debug)return;
...
function detectControllers(){
  ...
  controllerManager.activationUntil=performance.now()+20000;const button=qs('#activateControllerBtn');if(button){button.disabled=true;button.textContent='Scanning… press the D-pad'}updateControllerDebug(true);if(controllerManager.activationTimer)clearInterval(controllerManager.activationTimer);
...
function showDetails(g){
  ...
  const rewards=campaign?'3 AG Coins for each new level, 5 bonus coins at levels 10, 20, and 30, and a 100-coin bonus at level 40.':'3 AG Coins per win and a one-time 5-coin first-win bonus.';
...
function updateResumeButton(){const b=qs('#resumeSavedBtn'),a=account();if(!b)return;const cp=a?.resumeSession,g=cp&&ARCADE_GAMES.find(x=>x.id===cp.gameId);b.classList.toggle('hidden',!g);if(g)b.textContent=`▶ Resume ${g.name} — Level ${cp.level}`}
...
function startGame(g,level,mode=null,options={}){
  ...
  currentGame=g;showView('game');qs('#gameTitle').textContent=g.name;qs('#gameLevelText').textContent=campaign?`Level ${level} of ${g.levels}`:(g.id==='drawing-studio'||g.id==='music-sequencer'?'Creative Mode':'Single Match');selectedGameMode=mode;session=new GameSession(g,level,mode);configureControlDock(g);session.start();
}
function exitGame(){
...
class GameSession{
  constructor(game,level,mode='1P'){
    this.game=game;this.level=level;this.mode=mode;this.canvas=qs('#gameCanvas');this.ctx=this.canvas.getContext('2d');this.keys=new Set();this.running=false;this.paused=false;this.homePaused=false;this.completed=false;this.reward=0;this.last=0;this.elapsed=0;this.pointer={x:0,y:0,down:false};this.pointerGame=POINTER_GAME_ENGINES.has(game.engine);this.gamepadPointer={x:W/2,y:H/2,down:false,visible:false};this.gamepadKeys=new Set();this.prevGamepadButtons={};this.resultTimer=null;this.ready=true;this.readyUntil=0;this.readyShown=-1;this.touchStart=null;this.touchStarts=new Map();this.activePointers=new Map();this.readyHeldKeys=new Set();const userScale=({'very-slow':.78,relaxed:.9,standard:1,challenge:1.12}[account()?.settings.gamePace||'standard']||1);this.paceScale=userScale*(ENGINE_SPEED_SCALE[game.engine]||1);
    this.boundPointer=e=>this.handlePointer(e);this.boundPointerUp=e=>this.handlePointerUp(e);this.engine=createGameEngine(this);this.pointerGame=this.pointerGame||typeof this.engine.pointer==='function';
  }
  start(){this.running=true;this.canvas.addEventListener('pointerdown',this.boundPointer);this.canvas.addEventListener('pointermove',this.boundPointer);window.addEventListener('pointerup',this.boundPointerUp);window.addEventListener('pointercancel',this.boundPointerUp);this.engine.init?.();this.last=performance.now();this.beginReadyCountdown();requestAnimationFrame(t=>this.loop(t))}
  beginReadyCountdown(){this.ready=true;this.readyUntil=performance.now()+5000;this.readyShown=-1;this.updateReadyOverlay()}
  updateReadyOverlay(now=performance.now()){if(!this.ready)return;const remaining=Math.max(0,Math.ceil((this.readyUntil-now)/1000)),o=qs('#gameOverlay');if(remaining!==this.readyShown){this.readyShown=remaining;o.innerHTML=`<div class="overlay-card ready-card"><h2>${remaining>0?'Get Ready':'GO!'}</h2><p>${remaining>0?remaining:'Start!'}</p></div>`;o.classList.remove('hidden')}if(now>=this.readyUntil){this.ready=false;this.last=now;for(const key of this.readyHeldKeys){if(!this.keys.has(key))this.engine.keyDown?.(key);this.keys.add(key)}this.readyHeldKeys.clear();setTimeout(()=>{if(this.running&&!this.paused&&!this.completed)qs('#gameOverlay').classList.add('hidden')},350)}}
  ...
  pollGamepadPointer(pad,dir){if(!pad||!this.pointerGame)return;const p=this.gamepadPointer,move=dir||gamepadDirections(pad),rx=move.left?-1:move.right?1:0,ry=move.up?-1:move.down?1:0;if(rx||ry){p.visible=true;p.x=clamp(p.x+rx*11,0,W);p.y=clamp(p.y+ry*11,0,H);if(p.down)this.sendGamepadPointer('pointermove')}const click=gamepadButtonPressed(pad,0)||gamepadButtonPressed(pad,2)||Number(pad.buttons?.[7]?.value||0)>.45,old=!!this.prevGamepadButtons.pointerClick;if(click&&!old&&!this.paused&&!this.ready&&!this.completed){p.visible=true;p.down=true;this.sendGamepadPointer('pointerdown')}if(!click&&old)this.releaseGamepadPointer();this.prevGamepadButtons.pointerClick=click}
  ...
  pollGamepads(){
    if(this.completed||!this.running||this.ready)return;const pads=activeGamepads(),desired=new Set();const addSet=(dir,keys)=>{if(dir.left)desired.add(keys.left);if(dir.right)desired.add(keys.right);if(dir.up)desired.add(keys.up);if(dir.down)desired.add(keys.down)};
    ...
    const p1dir=mapPad(pads[0],0);if(this.mode==='2P'&&pads[1])mapPad(pads[1],1);this.pollGamepadPointer(pads[0],p1dir);for(const k of this.gamepadKeys)if(!desired.has(k))this.setKey(k,false);for(const k of desired)if(!this.gamepadKeys.has(k))this.setKey(k,true);this.gamepadKeys=desired;
  }
  loop(t){if(!this.running)return;if(this.ready)this.updateReadyOverlay(t);else this.pollGamepads();const dt=Math.min(.04,(t-this.last)/1000||0);this.last=t;if(!this.ready&&!this.paused&&!this.completed){const gameDt=dt*this.paceScale;this.elapsed+=gameDt;try{this.engine.update?.(gameDt)}catch(err){console.error(err);this.showError(err);return}}if(!this.running)return;try{this.engine.draw(this.ctx);this.drawGamepadPointer()}catch(err){console.error(err);this.showError(err);return}requestAnimationFrame(x=>this.loop(x))}
  pauseMarkup(){const g=this.game,campaign=!!g.campaign&&g.levels>1,rewards=campaign?'3 coins per new level; +5 at levels 10, 20, and 30; +100 at level 40.':'3 coins per win; +5 for the first win.';return`<div class="overlay-card pause-card"><h2>Game Paused</h2><h3>${escapeHtml(g.name)}</h3><p><b>Objective:</b> ${escapeHtml(objective(g))}</p><p><b>How to play:</b> ${escapeHtml(howTo(g))}</p><p><b>Controls:</b> ${escapeHtml(g.controls)}. Touch controls and direct canvas gestures are supported. Nintendo controller movement uses the D-pad only.</p><p><b>Rewards:</b> ${rewards}</p><button id="resumeBtn" class="primary">Resume</button><button id="restartBtn">Restart Level</button><button id="quitBtn">Save & Return to Launcher</button></div>`}
...
async function syncGameCatalog(){
  try{const response=await fetch('./games.json?v=11.4',{cache:'no-store'});if(!response.ok)return;const fresh=await response.json();if(!Array.isArray(fresh)||!fresh.length)return;const current=JSON.stringify(ARCADE_GAMES.map(g=>[g.id,g.name,g.engine,g.controls]));const next=JSON.stringify(fresh.map(g=>[g.id,g.name,g.engine,g.controls]));if(current!==next){ARCADE_GAMES.splice(0,ARCADE_GAMES.length,...fresh);if(activeAccountId){const selected=qs('#categorySelect').value;qs('#categorySelect').innerHTML='';renderLauncher();if([...qs('#categorySelect').options].some(o=>o.value===selected))qs('#categorySelect').value=selected;renderGames()}toast('The unique game catalog was refreshed')}}catch(error){console.warn('Catalog refresh skipped',error)}
}
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{
  const logo=qs('#mainLogo');if(logo&&!logo.complete)logo.addEventListener('error',()=>logo.src='./icon-512.png?v=11.4',{once:true});
  try{const registration=await navigator.serviceWorker.register('./service-worker.js?v=11.4');await registration.update();let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()})}catch(error){console.warn(error)}
  syncGameCatalog();
});
