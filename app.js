...
const APP_VERSION='10.3-web';
const DB_KEY='asArcadeMobileDB';
const AVATARS=AVATAR_PRESETS.map(p=>p.id);
const THEMES=['neon','sunset','forest','royal','sky','candy','ocean','lava','mint','galaxy','retro','midnight'];
const WALLPAPERS=['none','grid','stars','candy','waves','circuit','confetti','sunset','clouds','checker','aurora','space'];
const BANNERS=['Arcade Explorer','High Score Hero','Puzzle Master','Racing Champion','Creative Star','Family Game Night','Adventure Ace','Strategy Captain','Galaxy Gamer','Sports Legend','Cozy Player','Tech Wizard','Music Maker','Board Game Boss','Level Champion','Arcade Superstar'];
...
const STORE={
  themes:THEMES.map((name,i)=>({name,cost:i<2?0:140+i*25})),
  wallpapers:WALLPAPERS.map((name,i)=>({name,cost:i<2?0:120+i*25})),
  banners:BANNERS.map((name,i)=>({name,cost:i===0?0:100+i*20})),
...
function migrateDB(d){
  ...
  for(const a of d.accounts){
    ...
    if(!a.unlocks.themes.length)a.unlocks.themes=['neon'];if(!a.unlocks.wallpapers.length)a.unlocks.wallpapers=['none'];if(!a.unlocks.banners.length)a.unlocks.banners=[BANNERS[0]];
    a.settings=Object.assign({theme:'neon',wallpaper:'stars',reducedMotion:false,highContrast:false,sound:true,controlSize:'normal',textSize:'normal',cardSize:'normal',gamePace:'comfortable'},a.settings||{});
...
function newAccount(username,displayName,pinHash,type,avatar){return{id:uid(),username,displayName:displayName||username,pinHash,type,enabled:true,avatar:normalizeAvatar(avatar),coins:20,xp:0,banner:BANNERS[0],settings:{theme:'neon',wallpaper:'stars',reducedMotion:false,highContrast:false,sound:true,controlSize:'normal',textSize:'normal',cardSize:'normal',gamePace:'comfortable'},unlocks:{themes:['neon','sunset','forest'],wallpapers:['none','stars','grid'],banners:[BANNERS[0]],avatars:AVATARS.slice(0,4)},favorites:[],progress:{},achievements:[],stats:{plays:0,levels:0,completed:0,wins:0,seconds:0},daily:{date:'',plays:0,levels:0,claimed:false},parental:{minutes:0,multiplayer:true,spending:true,categories:[],external:true},createdAt:Date.now()}}
...
function applyTheme(){const a=account();document.body.className='';if(!a)return;document.body.classList.add(`theme-${a.settings.theme}`,`wallpaper-${a.settings.wallpaper}`,`controls-${a.settings.controlSize||'normal'}`,`text-${a.settings.textSize||'normal'}`,`cards-${a.settings.cardSize||'normal'}`);if(a.settings.highContrast)document.body.classList.add('high-contrast');if(a.settings.reducedMotion)document.body.classList.add('reduced-motion')}
...
function configureControlDock(g){
  ...
  if(g.players!==2)dock.classList.add('single-player');else dock.classList.add('multiplayer');
...
function startGame(g,level){
  ...
  qs('#gameInstructions').innerHTML=`<h3>${escapeHtml(g.name)} — Instructions</h3><p><b>Objective:</b> ${escapeHtml(objective(g))}</p><p><b>How to play:</b> ${escapeHtml(howTo(g))}</p><p><b>Computer controls:</b> ${escapeHtml(g.controls)}.</p><p><b>Touchscreen:</b> Use the fixed control dock at the bottom plus direct taps or drags on the game canvas where appropriate.</p><p><b>Gamepad:</b> Left stick/D-pad moves, A is the primary action, B is the secondary action, and Start pauses.</p><p><b>Game pace:</b> ${escapeHtml((a.settings.gamePace||'comfortable').replace(/^./,c=>c.toUpperCase()))}. Change it under Customize & Accessibility.</p><p><b>Rewards:</b> 1 AG Coin for each newly completed level and 5 bonus coins for finishing the game.</p>`;
...
class GameSession{
  constructor(game,level){
    this.game=game;this.level=level;this.canvas=qs('#gameCanvas');this.ctx=this.canvas.getContext('2d');this.keys=new Set();this.running=false;this.paused=false;this.completed=false;this.reward=0;this.last=0;this.elapsed=0;this.pointer={x:0,y:0,down:false};this.gamepadKeys=new Set();this.prevGamepadButtons={};this.resultTimer=null;this.paceScale=({'very-slow':.20,relaxed:.30,comfortable:.40,standard:.55,challenge:.70}[account()?.settings.gamePace||'comfortable']||.40);
  ...
  showResult(win,score){const o=qs('#gameOverlay'),next=this.level<this.game.levels,title=win?'Level Complete!':'Level Restart';o.innerHTML=`<div class="overlay-card"><h2>${title}</h2><p>Score: ${Math.round(score)}</p>${win?'':`<p>Restarting Level ${this.level} automatically. Your campaign progress is not reset.</p>`}${win?`<p>AG Coins earned: ${this.reward}</p>`:''}<button id="resultMain" class="primary">${win&&next?'Next Level':win?'Replay Level':'Restart Now'}</button><button id="resultLauncher">Return to Launcher</button></div>`;o.classList.remove('hidden');const restart=()=>{const g=this.game,l=win&&next?this.level+1:this.level;this.stop();session=new GameSession(g,l);qs('#gameLevelText').textContent=`Level ${l} of ${g.levels}`;session.start()};qs('#resultMain').onclick=restart;qs('#resultLauncher').onclick=exitGame;if(!win)this.resultTimer=setTimeout(()=>{if(session===this&&this.completed)restart()},2200)}
...
async function syncGameCatalog(){
  try{const response=await fetch('./games.json?v=10.3',{cache:'no-store'});if(!response.ok)return;const fresh=await response.json();if(!Array.isArray(fresh)||!fresh.length)return;const current=JSON.stringify(ARCADE_GAMES.map(g=>[g.id,g.name,g.engine,g.controls]));const next=JSON.stringify(fresh.map(g=>[g.id,g.name,g.engine,g.controls]));if(current!==next){ARCADE_GAMES.splice(0,ARCADE_GAMES.length,...fresh);if(activeAccountId){const selected=qs('#categorySelect').value;qs('#categorySelect').innerHTML='';renderLauncher();if([...qs('#categorySelect').options].some(o=>o.value===selected))qs('#categorySelect').value=selected;renderGames()}toast('The unique game catalog was refreshed')}}catch(error){console.warn('Catalog refresh skipped',error)}
}
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{
  const logo=qs('#mainLogo');if(logo&&!logo.complete)logo.addEventListener('error',()=>logo.src='./icon-512.png?v=10.3',{once:true});
  try{const registration=await navigator.serviceWorker.register('./service-worker.js?v=10.3');await registration.update();let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()})}catch(error){console.warn(error)}
  syncGameCatalog();
});
