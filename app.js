...
const MULTIMODE_ENGINES=new Set(['pong','soccer','dotsboxes','volleyball','airhockey','tictactoe','connect4','reversi','checkers','mancala','gomoku','hex']);
...
const ACHIEVEMENTS=[
 ...
 ['Game Sampler','Try 10 different games',a=>gameCount(a)>=10],['Genre Explorer','Try 20 different games',a=>gameCount(a)>=20],['Arcade Explorer','Try 35 different games',a=>gameCount(a)>=35],['Library Traveler','Try 50 different games',a=>gameCount(a)>=50],['Played Them All','Try all 75 games',a=>gameCount(a)>=75],
...
function migrateDB(d){
  ...
  for(const a of d.accounts){
    ...
    if(a.settings.speedBalanceVersion!=='10.7'){a.settings.gamePace='standard';a.settings.speedBalanceVersion='10.7';changed=true}
...
function supportsMultipleModes(g){return MULTIMODE_ENGINES.has(g.engine)}
function showDetails(g){
  ...
  const modeMarkup=multi?`<label>Play mode<select id="gameModeSelect"><option value="1P" selected>Single player vs computer</option><option value="2P">Two players on this device</option></select></label>`:'';
  ...
  qs('#detailsContent').innerHTML=`<div style="font-size:3rem">${CATEGORY_ICONS[g.category]||'🎲'}</div><h2>${escapeHtml(g.name)}</h2><p><b>${escapeHtml(g.category)}</b> • ${format} • ${multi?'Single player or two players':g.players===2?'Two players':'One player'}</p>${modeMarkup}<div class="panel-card"><h3>Objective</h3><p>${escapeHtml(objective(g))}</p><h3>How to play</h3><p>${escapeHtml(howTo(g))}</p><h3>Controls</h3><p>${escapeHtml(g.controls)}. Touch controls stay at the bottom on phones and tablets; keyboard and gamepad controls also work on computers.</p><h3>Progress</h3><p>${progress}</p><p><b>Rewards:</b> ${rewards}</p></div><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:14px"><button id="startNew" class="primary">${campaign?'Start New Game':'Start'}</button>${campaign?`<button id="resumeGame">Resume Level ${Math.min(g.levels,p.level||1)}</button>`:''}<button id="detailFavorite">${a.favorites.includes(g.id)?'Remove Favorite':'Add Favorite'}</button></div>`;
  openDialog(detailsDialog);const chosen=()=>qs('#gameModeSelect')?.value||selectedGameMode;qs('#startNew').onclick=()=>{detailsDialog.close();startGame(g,1,chosen())};if(campaign)qs('#resumeGame').onclick=()=>{detailsDialog.close();startGame(g,Math.min(g.levels,p.level||1),chosen())};qs('#detailFavorite').onclick=()=>{toggleFavorite(g.id);detailsDialog.close()}
}
...
function renderSettings(out,a){
  ...
  const controls=qs('#settingsAvatarControls'),preview=qs('#settingsAvatarPreview');const redraw=()=>preview.innerHTML=avatarSVG(readAvatarControls(controls),164);const wire=()=>controls.querySelectorAll('select').forEach(el=>el.addEventListener('change',redraw));wire();qs('#randomizeAvatar').onclick=()=>{controls.innerHTML=avatarControlMarkup('settings-avatar',randomAvatarConfig());wire();redraw()};out.querySelectorAll('[data-avatar-preset]').forEach(btn=>btn.onclick=()=>{controls.innerHTML=avatarControlMarkup('settings-avatar',avatarPreset(btn.dataset.avatarPreset).config);wire();redraw()});
...
class GameSession{
  ...
  handlePointer(e){if(!this.running||this.paused||this.completed)return;const r=this.canvas.getBoundingClientRect();this.pointer.x=(e.clientX-r.left)*this.canvas.width/r.width;this.pointer.y=(e.clientY-r.top)*this.canvas.height/r.height;this.pointer.down=e.type==='pointerdown'||this.pointer.down;this.engine.pointer?.(this.pointer,e.type)}
  pollGamepads(){
  ...
  showResult(win,score){this.running=false;this.keys.clear();this.gamepadKeys.clear();this.pointer.down=false;const campaign=!!this.game.campaign&&this.game.levels>1,o=qs('#gameOverlay'),next=campaign&&this.level<this.game.levels,title=win?(campaign?'Level Complete!':'Match Complete!'):(campaign?'Level Restart':'Try Again');o.innerHTML=`<div class="overlay-card"><h2>${title}</h2><p>Score: ${Math.round(score)}</p>${win?'':`<p>${campaign?`Restarting Level ${this.level}`:'Restarting the match'} automatically. Your saved progress is not reset.</p>`}${win?`<p>AG Coins earned: ${this.reward}</p>`:''}<button id="resultMain" class="primary">${win&&next?'Next Level':win?(campaign?'Replay Level':'Play Again'):'Restart Now'}</button><button id="resultLauncher">Return to Launcher</button></div>`;o.classList.remove('hidden');const restart=()=>{const g=this.game,l=win&&next?this.level+1:this.level,mode=this.mode;this.stop();session=new GameSession(g,l,mode);qs('#gameLevelText').textContent=campaign?`Level ${l} of ${g.levels}`:(g.id==='drawing-studio'||g.id==='music-sequencer'?'Creative Mode':'Single Match');selectedGameMode=mode;configureControlDock(g);session.start()};qs('#resultMain').onclick=restart;qs('#resultLauncher').onclick=exitGame;if(!win)this.resultTimer=setTimeout(()=>{if(session===this&&this.completed)restart()},2200)}
...
async function syncGameCatalog(){
  try{const response=await fetch('./games.json?v=10.7',{cache:'no-store'});if(!response.ok)return;const fresh=await response.json();if(!Array.isArray(fresh)||!fresh.length)return;const current=JSON.stringify(ARCADE_GAMES.map(g=>[g.id,g.name,g.engine,g.controls]));const next=JSON.stringify(fresh.map(g=>[g.id,g.name,g.engine,g.controls]));if(current!==next){ARCADE_GAMES.splice(0,ARCADE_GAMES.length,...fresh);if(activeAccountId){const selected=qs('#categorySelect').value;qs('#categorySelect').innerHTML='';renderLauncher();if([...qs('#categorySelect').options].some(o=>o.value===selected))qs('#categorySelect').value=selected;renderGames()}toast('The unique game catalog was refreshed')}}catch(error){console.warn('Catalog refresh skipped',error)}
}
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{
  const logo=qs('#mainLogo');if(logo&&!logo.complete)logo.addEventListener('error',()=>logo.src='./icon-512.png?v=10.7',{once:true});
  try{const registration=await navigator.serviceWorker.register('./service-worker.js?v=10.7');await registration.update();let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()})}catch(error){console.warn(error)}
  syncGameCatalog();
});
